...
def CreateTheta(x, order):
    ...
    # order 1
    for i in range(xdim):
        Theta = np.concatenate((Theta, [x[i]]), axis=0)

    # order 2
    if order >= 2:
        for i in range(xdim):
            for j in range(i+1):    #　　木構造により場合の数を尽くす（ノート参照）
                Theta = np.concatenate((Theta, [x[i] * x[j]]), axis=0)
    ...
    if order >= 3:
        for i in range(xdim):
            for j in range(i+1):
                for k in range(j+1):
                    Theta = np.concatenate((Theta, [x[i] * x[j] * x[k]]), axis=0)
    ...
    if order >= 4:
        for i in range(xdim):
            for j in range(i+1):
                for k in range(j+1):
                    for l in range(k+1):
                        Theta = np.concatenate((Theta, [x[i] * x[j] * x[k] * x[l]]),
                                               axis=0)
            ...
            for j in range(i+1):
                for k in range(j+1):
                    for l in range(k+1):
                        for m in range(l+1):
                            Theta = np.concatenate(
                                (Theta, [x[i] * x[j] * x[k] * x[l] * x[m]]), axis=0)
